...
def create_context(content: str, language: str, file_path: str = "test.py") -> RuleContext:
...
class TestHardcodedSecretsRule:
    ...
    def test_detects_generic_api_key_pattern(self, rule):
        ...
        context = create_context(
            'api_key = "abcdef1234567890ghijklmnopqrs"',
            "python"
        )
    ...
    def test_detects_long_api_key(self, rule):
        context = create_context(
            'apikey = "aBcD1234567890EfGhIjKlMnOpQrS"',
            "python"
        )
        findings = rule.check(context)
        assert len(findings) >= 1
...
class TestMissingHTTPSRule:
    ...
    def test_detects_http_api_url(self, rule):
        context = create_context(
            'url = "http://api.production.com/api/users"',
            "python"
        )
        findings = rule.check(context)
        assert len(findings) >= 1
    ...
    def test_detects_http_auth_url(self, rule):
        context = create_context(
            'login_url = "http://myapp.io/auth/login"',
            "python"
        )
        findings = rule.check(context)
        assert len(findings) >= 1
...
class TestPathTraversalRule:
    ...
    def test_detects_fs_readfile_with_dynamic_path(self, rule):
        context = create_context(
            'fs.readFile(path + userInput, callback)',
            "javascript"
        )
        findings = rule.check(context)
        assert len(findings) >= 1
...
class TestInsecureDeserializeRule:
    ...
    @pytest.fixture
    def rule(self):
        from claude_indexer.rules.security.insecure_deserialize import InsecureDeserializeRule
    ...
    def test_detects_node_serialize(self, rule):
        context = create_context(
            'serialize.unserialize(userData)',
            "javascript"
        )
        findings = rule.check(context)
        assert len(findings) >= 1
...
class TestSensitiveExposureRule:
    ...
    @pytest.fixture
    def rule(self):
        from claude_indexer.rules.security.sensitive_exposure import SensitiveExposureRule
